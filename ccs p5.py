# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 08:20:10 2019
...
def mean(L):
    ...
    return s/len(L)
##############################################################################################################
mydir = "D:/Cours/L3 Chimie/L3 PCM Stage fin annee/ccs/ccs p5/"
import os
...
for i in range(len(lfil)):
    ...
    newtxt.close()
    titre.append("Dimère "+lfil[i][11:-6]+" cluster "+lfil[i][-5])
print("Opening done")
...
for i in range(len(table)):
    print("---------------------------------------------------------------------------------")
    print(titre[i])
    plt.hist(table[i], 20, facecolor = color[0],edgecolor = 'black')
    plt.show()
    print(mean(table[i]))
